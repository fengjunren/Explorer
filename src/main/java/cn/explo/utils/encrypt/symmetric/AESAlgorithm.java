package cn.explo.utils.encrypt.symmetric;
...
import cn.explo.utils.encrypt.codec.Base64Algorithm;
import cn.explo.utils.encrypt.codec.Hex;
...
import cn.explo.utils.encrypt.util.EncryptStringUtils;

/**
 * Nothing seek, Nothing find. author: shuzhilong Date: 2017/12/29 下午2:41 desc:
 * (The role of this class is to ...) To change this template use preferences |
...
//    private int transStringMode;
	...
	public String encrypt(String data, String encryptKey, String ivString) throws Exception {
		...
		} catch (UnsupportedEncodingException | BadPaddingException | IllegalBlockSizeException | InvalidKeyException
	...
	public void checkParams(String method, String data, String encryptKey) {
		...
		if (encryptKey.length() != keyLength) {
			throw new RuntimeException(
					"[AESAlgorithm]" + method + "rsakeySize is " + encryptKey.length() + " [need] " + keyLength);
		}
	}
	...
	public Cipher buildCipher(String encryptKey, String ivString, int securityMode) throws Exception {
		...
		if (cipherAlgorithm.equals(AesCipherAlgorithmEnum.ECB_PKCS5PADDING.getCipherAlgorithm())) {
			...
			if (ivString.length() != keyLength) {
				throw new RuntimeException(
						"[AESAlgorithm] [decrypt] ivString is " + ivString.length() + " [need] " + keyLength);
			}
...
//    }
	...
	private Key getKey(String key) throws UnsupportedEncodingException {
		SecretKeySpec secretKeySpec = new SecretKeySpec(key.getBytes(encodingCharset), "AES");
		return secretKeySpec;
	}
